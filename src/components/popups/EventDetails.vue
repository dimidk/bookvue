...
function updateEvent(arg) {
  if (
    props.bookuser !== eventToShow.bookusername &&
    isAdmin.value === "false"
  ) {
    ...
    if (title.value !== "") {
      ...
      console.log(
        "change only the title, so the date is the same",
        eventToShow.start,
        eventToShow.end,
      );
    ...
    console.log(
      "event that is going to be updated ",
      eventToShow.id,
      eventToShow.bookusername,
      eventToShow.labusername,
    );
    console.log(
      "event with updated datetime",
      eventToShow.start,
      eventToShow.end,
    );
  }
...
async function updateDate() {
  ...
  let response = await axiosInstance.post(reqUrl, eventToShow);
  if (response.status === 200) console.log("date updated");
  ...
  <div>
    <dialog ref="dialog" v-if="event">
      <fieldset>
        ...
        <fieldset><b> Τίτλος:</b> {{ eventToShow.title }}</fieldset>
        <fieldset><b> Έναρξη: </b>{{ eventToShow.start }}</fieldset>
        <fieldset><b> Λήξη: </b>{{ eventToShow.end }}</fieldset>

        <fieldset>
          <b> Κράτηση από: </b> {{ eventToShow.bookusername }}
        </fieldset>
        <fieldset><b> Εργαστήριο: </b> {{ eventToShow.labname }}</fieldset>
        <fieldset><b> Υπεύθυνος: </b> {{ eventToShow.labusername }}</fieldset>
      </fieldset>
      <hr />
