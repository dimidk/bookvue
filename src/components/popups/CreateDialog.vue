...
async function addEventForm() {
  console.log(
    "newbooking " + newBooking.bookusername + " " + newBooking.labusername,
  );
  console.log(
    "detail transfer " +
      props.details.id +
      ...
      props.details.end +
      " " +
      props.details.labname,
  );
  console.log(
    "newbooking continue data details " +
      newBooking.id +
      " " +
      newBooking.start +
      " " +
  ...
  if (recursionFields.value === true) {
    ...
    let recurPostUrl =
      "/api/repeat_booking/" + newStartDate.value + "/" + newEndDate.value;
