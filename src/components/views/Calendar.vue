...
async function changeTimeSlot(arg) {
  ...
  console.log(
    "all info about modified event " + recordBook.start + " " + recordBook.end,
  );
  ...
  try {
    ...
    //
    if (
      recordBook.bookusername !== data.bookusername &&
      isAdmin.value === "false"
    ) {
...
async function showDetails(arg) {
  ...
  console.log(
    "show details start date in string and bookusername " +
      details.start +
      " " +
      details.bookusername +
      " " +
      details.labname,
  );
